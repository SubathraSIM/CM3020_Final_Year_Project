...
DATABASE_PATH = (
    DATABASE_FOLDER / "wellbeing_system.db"
)
SCHEMA_PATH = (
    ...
    with connect() as connection:
        ...
        if (
            ...
            connection.execute(
                """
                ALTER TABLE users
                ADD COLUMN consent_accepted_at TEXT
                """
            )
            ...
            WHERE
                ...
                AND current_row.id IN (
                    ...
                    WHERE
                        grouped_row.user_id = ?
                    GROUP BY date(
                        grouped_row.created_at,
                        'localtime'
                    )
                )
            ORDER BY
                current_row.created_at DESC,
                current_row.id DESC
            LIMIT ?
            """,
            (
